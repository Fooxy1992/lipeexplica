...
import { isActiveSubscription } from "@/core/domain/entities/subscription";
...
export default async function AccountPage() {
  ...
  const [profile, purchases, subscriptions, products] = await Promise.all([
    c.profiles.findById(user.id),
    ...
    <div className="min-h-dvh bg-[#EEF2FA]">
      <header className="sticky top-0 z-30 border-b border-[#E4EAF4] bg-white/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-6 py-4">
          <Link
            href="/library"
            className="inline-flex items-center gap-2 text-xs font-medium text-[#8B92A8] transition hover:text-[#1C1E2E]"
          >
