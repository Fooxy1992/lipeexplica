...
export default function RaffleClient({ raffle, initialPaid, initialReserved }: Props) {
  ...
  return (
    <div className="relative min-h-dvh overflow-hidden bg-[#08090a] px-4 py-16">
      <div
        aria-hidden
      ...
      <div className="relative mx-auto max-w-lg space-y-6">
        {/* Header */}
        <div className="rounded-3xl border border-white/[0.08] bg-[#121314] p-8 text-center text-white">
          <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[#ff4b2b]/10">
          ...
          <div className="mt-6 space-y-2">
            ...
            <div className="h-3 overflow-hidden rounded-full bg-white/[0.08]">
              <div
                className="h-full rounded-full transition-all duration-700 bg-[#ff4b2b]"
                style={{ width: `${paidPct}%` }}
              />
            </div>
          ...
          {status.status === 'completed' && status.winner_ticket_id && (
            <div className="mt-5 flex items-center justify-center gap-2 rounded-xl border border-[#ff4b2b]/20 bg-[#ff4b2b]/10 px-4 py-3 text-sm text-[#ff4b2b]">
        ...
        {/* Pricing tiers */}
        <div className="rounded-2xl border border-white/[0.08] bg-[#121314] p-5 text-white">
          <p className="mb-3 text-sm font-semibold text-white/80">Faixas de preço</p>
          <div className="grid grid-cols-2 gap-2 text-sm text-white/60">
            <div className="rounded-lg bg-white/[0.04] px-3 py-2">1–2 ações · <span className="text-white/90">R$ 25/un</span></div>
            <div className="rounded-lg bg-white/[0.04] px-3 py-2">3–4 ações · <span className="text-white/90">R$ 20/un</span></div>
            <div className="rounded-lg bg-white/[0.04] px-3 py-2">5–9 ações · <span className="text-white/90">R$ 18/un</span></div>
            <div className="rounded-lg bg-white/[0.04] px-3 py-2">10+ ações · <span className="text-white/90">R$ 15/un</span></div>
          </div>
        </div>

        {/* Buy form */}
        {soldOut ? (
          <div className="rounded-2xl border border-white/[0.08] bg-[#121314] p-8 text-center text-white">
          ...
          <form
            ...
            className="rounded-3xl border border-white/[0.08] bg-[#121314] p-8 text-white"
          >
            ...
            <div className="mb-4">
                ...
                {[1, 3, 5, 10].map((n) => (
                  <button
                    ...
                    className={`flex-1 rounded-xl border py-2 text-sm font-medium transition ${
                      qty === n
                        ? 'border-[#ff4b2b]/60 bg-[#ff4b2b]/15 text-[#ff4b2b]'
              ...
              <input
                ...
                className="mt-2 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none focus:border-[#ff4b2b]/40 focus:ring-1 focus:ring-[#ff4b2b]/20"
                placeholder="Ou digita a quantidade"
              />
            </div>

            {/* Price preview */}
            <div className="mb-5 rounded-xl border border-[#ff4b2b]/20 bg-[#ff4b2b]/[0.08] px-4 py-3">
              <div className="flex justify-between text-sm">
                <span className="text-white/70">{qty} × {formatBRL(unitPriceForQty(qty))}</span>
                <span className="font-bold text-[#ff4b2b]">{formatBRL(priceForQty(qty))}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none focus:border-[#ff4b2b]/40 focus:ring-1 focus:ring-[#ff4b2b]/20"
              />
              <input
                required
                type="email"
            ...
            <button
              ...
              className="mt-5 w-full rounded-full bg-[#ff4b2b] py-3.5 text-sm font-bold text-white transition hover:brightness-110 active:scale-[0.98] disabled:opacity-50"
            >
